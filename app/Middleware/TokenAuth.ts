...
const TOKENS: Record<
  string,
  { role: "limited" | "all-access"; owner?: string }
> = {
...
export default class TokenAuth {
  public async handle(
    ...
    // Accept token via `authorization: Bearer <token>`, `x-api-key`, or `x-api-token`.
    const authHeader = ctx.request.header("authorization") || "";
    const apiKey = ctx.request.header("x-api-key");
    const apiTokenHeader = ctx.request.header("x-api-token");

    let token = "";
    if (authHeader.toLowerCase().startsWith("bearer "))
      token = authHeader.slice(7).trim();
    else if (apiKey) token = apiKey;
    else if (apiTokenHeader) token = apiTokenHeader;
